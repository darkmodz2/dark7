...
def handle_key_generation(message):
    ...
    if len(args) != 2 or args[1].lower() not in DURATION_MAP:
        valid_durations = ', '.join(DURATION_MAP.keys())
        bot.send_message(message.chat.id, f"*Use like:* /key 1 hour\n*Valid options:* {valid_durations}", parse_mode='Markdown')
        return

    duration = args[1].lower()
    price, days = DURATION_MAP[duration]
    key = f"{duration}-" + ''.join(random.choices('abcdefghijklmnopqrstuvwxyz0123456789', k=10))
...
def process_redeem_key(message):
    ...
    try:
        ...
        if duration not in DURATION_MAP:
            raise ValueError("Invalid duration in key")
